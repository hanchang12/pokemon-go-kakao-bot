...
원본 사이트는 영문이라, 캐시에 저장하기 전에 NVIDIA로 포켓몬 이름을 한국 공식 명칭(메가/섀도/폼 접두어 포함)으로 한 번에 번역합니다 - 채팅 요청마다가 아니라 이 월간 갱신 시점에만 호출되므로 응답 속도엔 영향이 없습니다. 번역이 실패하면 그 이름만 원문(영문) 그대로 캐시됩니다.

같은 캐시 데이터를 `/포고봇 티어 [타입]`으로 직접 조회할 수도 있습니다 - `/포고봇 상성`처럼 AI를 거치지 않고 즉시 응답하므로, 순위만 궁금하면 AI 질문보다 이쪽이 더 빠르고 정확합니다.
