...
위 명령어에 없는 질문도 `/포고봇 ...`으로 물어보면 등록된 일정을 근거로 AI가 답합니다. 수집용 `AI_PROVIDER` 설정과 무관하게 항상 NVIDIA 무료 티어(`NVIDIA_MODEL`, 기본값 `meta/llama-3.2-11b-vision-instruct`)를 씁니다 - 수집용 Gemini/기타 할당량을 안 쓰려는 목적입니다. 전달 방식은 `/포고봇 수집`과 동일하게 두 단계 블로킹 호출(`/api/messages` 즉시 응답 + `await_ask: true` → `POST /api/ask` 긴 타임아웃 호출)입니다.
