...
`x-admin-token`으로 `/api/admin/collect`를 직접 호출하지 않아도, 방에서 `포고봇 수집`을 보내면 바로 실행됩니다. 관리자 API와 달리 별도 인증은 없습니다 - 이 명령을 받으면 응답만 즉시 오고(`🔄 이벤트 수집을 시작했어요...`) 실제 수집은 백그라운드 스레드에서 진행되며(메신저봇R의 짧은 요청 타임아웃보다 수집이 오래 걸릴 수 있어서), 끝나면 결과가 예약 발송과 같은 큐(`/api/subscriptions/due`)를 통해 그 방으로 다시 전달됩니다. 이미 진행 중인 수집이 있으면 새로 시작하지 않고 "이미 진행 중" 안내만 돌려줍니다.
...
서버(FastAPI)는 카카오톡 방에 직접 메시지를 보낼 수 없고, 오직 메신저봇R 스크립트만 보낼 수 있습니다. 그래서 구조는 다음과 같습니다: 서버는 예약 시각이 된 항목을 `GET /api/subscriptions/due` 큐에 담아두고, `messenger-bot/pogo-bot.js`가 `POLL_INTERVAL_MS`(기본 1분)마다 이 엔드포인트를 폴링해서 `bot.send(room, message)`로 각 방에 직접 전달합니다. **즉 예약 발송이 동작하려면 메신저봇R 앱에서 이 봇이 계속 켜져 있어야 합니다.**
