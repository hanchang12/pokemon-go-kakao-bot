...
- `/포고봇 예약 09:00` (1회), `/포고봇 예약 매일 09:00` (정기) — 오늘의 일정 전체
- `/포고봇 예약 레이드아워 17:50`, `/포고봇 예약 스포트라이트 17:50` — 해당 종류만 매주 알림
- `/포고봇 예약확인` (이 방의 모든 예약 확인), `/포고봇 예약취소` (전체 취소), `/포고봇 예약취소 레이드아워` (해당 종류만 취소)
...
방 하나에 `digest`(오늘의 일정 전체), `raid_hour`(레이드아워), `spotlight_hour`(스포트라이트) 세 종류를 동시에 예약할 수 있습니다 - 내부적으로 `(room, kind)` 조합이 유니크 키라 서로 독립적으로 등록·확인·취소됩니다. 레이드아워/스포트라이트는 게임 내에서 요일이 고정(매주 수/목)이라, 예약 시각의 요일도 그 요일로 자동 맞춰지고 정기 예약은 7일 주기로 돌아갑니다(일반 `digest` 예약은 기존처럼 1일 주기). 발송 문구도 종류에 맞게 그날의 해당 카테고리 이벤트만 골라 보여줍니다.
...
원본 사이트는 영문이라, 캐시에 저장하기 전에 포켓몬 이름을 한국 공식 명칭으로 번역합니다. 처음엔 NVIDIA(AI)로 시켰는데, 한국 정발명이 음역이 아니라 완전히 다른 창작명인 경우가 많아서(예: Blastoise → "블라스터"가 아니라 "거북왕") 모델이 그럴듯하지만 틀린 이름을 지어내는 사고가 실제로 있었습니다(더 큰 모델로 바꿔도 마찬가지). 그래서 [PokeAPI](https://pokeapi.co)(포켓몬 공식 다국어 이름을 제공하는 무료 오픈 데이터베이스)에서 기본 종 이름만 정확히 조회하고, Mega/Shadow 같은 접두어는 항상 같은 규칙이라 기계적으로 붙입니다(`app/pokeapi.py`, `app/tier_source.py`). Therian Forme 같은 복잡한 폼 설명은 번역 없이 영문 그대로 남습니다 - 완벽하진 않지만 최소한 포켓몬 자체를 잘못 알려주는 일은 없습니다.

같은 캐시 데이터를 `/포고봇 티어 [타입]`으로 직접 조회할 수도 있습니다 - `/포고봇 상성`처럼 AI를 거치지 않고 즉시 응답하므로, 순위만 궁금하면 AI 질문보다 이쪽이 더 빠르고 정확합니다.

## 진화 체인

`/포고봇 진화 [영문이름]`은 PokeAPI에서 해당 포켓몬의 진화 체인을 조회해 한국 공식 명칭으로 답합니다(예: `charmander` → "파이리 → 리자드 → 리자몽"). 이브이처럼 갈라지는 경우 같은 단계에 있는 포켓몬을 `/`로 묶어서 보여줍니다. 체인이 보통 1~3단계라 PokeAPI 응답도 빨라서(NVIDIA와 달리) AI 없이 바로 동기로 답합니다. 현재는 영문 이름만 입력받습니다 - 한국어 이름 입력을 지원하려면 전체 포켓몬(1000종 이상)의 한글명 역인덱스를 미리 구축해야 해서 아직 범위 밖입니다.
