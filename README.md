...
## 예약 (현재 서버에만 등록됨 - 실제 발송 연결은 미완성)

방마다 `포고봇 예약 09:00`(오늘/내일 1회) 또는 `포고봇 예약 매일 09:00`(매일 정기)으로 예약을 등록/확인(`포고봇 예약확인`)/취소(`포고봇 예약취소`)할 수 있습니다. 방당 예약은 하나이며, 다시 예약하면 이전 예약을 덮어씁니다.

서버(FastAPI)는 카카오톡 방에 직접 메시지를 보낼 수 없고, 오직 메신저봇R 스크립트만 보낼 수 있습니다. 예약 시각이 된 항목은 서버의 `GET /api/subscriptions/due`에 큐로 쌓이는 것까지는 동작합니다 - **다만 이를 실제로 꺼내서 방에 전달하는 폴링 로직(`pogo-bot.js`의 `setInterval`)은 메신저봇R 스크립트 엔진에서 기존 명령 처리(`response()`)까지 먹통으로 만드는 문제가 있어 되돌렸습니다.** 스크립트는 다시 예전 버전(`git show 9bf7461:messenger-bot/pogo-bot.js`)으로 복원된 상태라, 지금은 예약을 등록해도 실제로 방에 메시지가 오지는 않습니다. 안전한 전달 방식(예: `response()` 호출 시 "묻어가는" 방식으로 due 항목을 같이 확인하는 등)을 다시 설계해야 합니다.
